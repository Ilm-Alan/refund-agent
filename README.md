...
Real CRM and payment integrations behind the same tool interface, persistence for sessions and the audit log, authentication separating the two views, token-level streaming for the chat, a human review queue behind the escalation path, and an eval suite that replays adversarial conversations against the gate. The policy engine and dispatcher boundaries are already shaped for that swap: tools are the only place the agent touches state.
